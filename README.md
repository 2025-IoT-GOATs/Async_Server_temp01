...
.
.
"CHAT" -> "LOGIN" "CHAT" 두가지로 분리
```
기존 CHAT 으로 분기하여 LOGIN 과 CHAT 기능 두가지를 담당했었는데, 멀티플레이 지원을 위해 로그인시 서버에 접속중인 기존 플레이어와, 새로접속한 플레이어 동기화가 필요해졌다.
기존방식대로 진행해도 되었으나, 유지보수나 가독성 측면에서 코드가 길어지는 바람에, LOGIN 아이디를 추가하여 기능을 분리해주었다.

LOGIN 패킷 아이디 주요기능
- 새로운 플레이어 로그인 -> 플레이어관리자에 플레이어 등록 -> 클라에서 본인 아이디 인식을 위해 맨 처음 본인의 아이디를 전송해주었다, 이 때 순서를 보장하기위해 1:1 통신으로 본인의 아이디를 송신
- 브로드캐스팅 -> 새 플레이어가 관리자 풀에 등록됨 -> 현재 모든 플레이어 정보를 브로드캐스팅
- 클라이언트 예외처리 -> 이미 존재하는 플레이어는 새로생성X , 새로운 플레이어만 새로생성
